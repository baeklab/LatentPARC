...
# ── Training ──────────────────────────────────────────────────────────────────
EPOCHS              = 3000
SAVE_RATE           = 200        # Save state dict every X epochs
LEARNING_RATE       = 1e-5
LOSS_WEIGHTS        = [0, 1.0, 1.0]    # [reconstruction, latent, rollout]
MODE                = "rollout_train"
...
AE_WEIGHTS_PATH     = "autoencoder/weights/SRH_frozenBasicAE3000onsingleEMdata_SigOut_FFTLoss_SRHfinetune_onShahabData_1000.pth"
SAVE_PATH           = f"{BASE_PATH}/LatentPARC/latent_parc/PARCtorch/PARCtorch/LatentPARC/weights"
WEIGHTS_NAME        = "SRH_frozenBasicAE3000onsingleEMdata_SigOut_FFTLoss_SRHfinetune_onShahabData_rollout5_lr1e5_NoScheduler_bs1"

# ── Flags ─────────────────────────────────────────────────────────────────────
FREEZE_AE           = True       # Freeze autoencoder weights during training
LOAD_WEIGHTS        = False      # Load checkpoint to resume training
LOAD_WEIGHTS_PATH   = "name.pth" # Path to checkpoint (only used if LOAD_WEIGHTS=True)
...
# Load pretrained SuperResHeadAutoencoder weights
sr_ae = SuperResHeadAutoencoder(layers=LAYER_SIZES, latent_dim=LATENT_DIM, act_fn=nn.ReLU(), out_act=nn.Sigmoid()).to(device) #out_act=nn.Sigmoid()
ckpt = torch.load(AE_WEIGHTS_PATH, map_location=device)
sr_ae.load_state_dict(ckpt["model_state_dict"])

# Extract encoder and decoder submodules
encoder = sr_ae.encoder
decoder = sr_ae.decoder
sr_head = sr_ae.sr_head
...
if FREEZE_AE:
    ...
    for param in decoder.parameters():
        param.requires_grad = False
    for param in sr_head.parameters():
        param.requires_grad = False
...
model_init = lp_model_sr(encoder, decoder, differentiator, integrator, sr_head=sr_head).to(device)
...
# Scheduler options (uncomment to enable):
...
if LOAD_WEIGHTS:
    ckpt = torch.load(LOAD_WEIGHTS_PATH, map_location=device)
