...
data_dirs = ["/usr/WS1/pcrom/for_Zoe/final_preprocess/large_train"]
...
data_dir_train = "/usr/WS1/pcrom/for_Zoe/final_preprocess/large_train"  
...
save_path="/g/g16/gray65/LatentPARC/latent_parc/PARCtorch/PARCtorch/3AE_LatentPARC"
weights_name="large_LP_1899_plus"
...
differentiator = Differentiator(latent_dim=latent_dim).to(device)
...
AE_weights_path = "autoencoder/weights/large_AE_3000.pth"
...
optimizer = Adam(model_init.parameters(), lr=1e-4)

load_weights = True

if load_weights == True:
    ckpt_path = "large_LP_1899.pth"
    ckpt = torch.load(ckpt_path, map_location=device)

    model_init.load_state_dict(ckpt["model_state_dict"])
    optimizer.load_state_dict(ckpt["optimizer_state_dict"])

    ## re-freeze AE weights
    for param in encoder.parameters():
        param.requires_grad = False
...
log_dict = model.train(criterion, epochs=1101, image_size = [64, 160], n_channels=3, device=device, 
