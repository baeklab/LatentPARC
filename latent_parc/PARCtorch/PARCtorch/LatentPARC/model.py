# TODO: I think we should get rid of the wrapper, figure out when wrapper would be useful and decide whether to keep
...
from autoencoder.loss_funcs import FFTLoss  # adjust to wherever FFTLoss is defined
...
class lp_model_sr(nn.Module):
    def __init__(self, encoder, decoder, differentiator, integrator, sr_head=None):
        super().__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.differentiator = differentiator
        self.integrator = integrator
        self.sr_head = sr_head  # optional, stays compatible with original if None
    ...
    def forward(self, x, n_ts=1, mode='train'):
        if mode == 'train':
            if n_ts == 0:
                ...
                decoded = self.sr_head(blurry) if self.sr_head else blurry
            ...
            z_i = self.encoder(x)
            z_list.append(z_i)
            blurry = self.decoder(z_i)
            decoded_list.append(self.sr_head(blurry) if self.sr_head else blurry)
...
class LatentPARC:
    ...
    def train(self, loss_function, epochs, save_every_epochs, image_size, n_channels, device, train_loader, val_loader, scheduler=None, noise_fn=None, initial_max_noise=0.16, n_reduce_factor=0.5, ms_reduce_factor=0, reduce_on=1000, loss_weights=[1.0, 1.0, 1.0], mode="single_ts_train"):
        """Wrapper for training."""
        return train_latentparc(self.network, self.optimizer, loss_function, train_loader, val_loader,
                                device, epochs, save_every_epochs, image_size, n_channels, scheduler, noise_fn,
...
def train_latentparc(model, optimizer, loss_function, train_loader, val_loader, device, 
                     epochs=10, save_every_epochs = 100, image_size=(128, 256), n_channels=3,
                     scheduler=None, noise_fn=None, initial_max_noise=0.16, n_reduce_factor=0.5,
    ...
    for epoch in range(epochs):
        ...
        for images in tqdm(train_loader, desc="Training"):
            ...
            if mode == "single_ts_train":
                ...
                L_2 = loss_function(z_hat, z_hat_next, use_fft=False) if isinstance(loss_function, FFTLoss) else loss_function(z_hat, z_hat_next)
                ...
                L_1 = loss_function(x_hat[0, ...], ic)
                L_2 = loss_function(z_hat[1:, ...], z_hat_next, use_fft=False) if isinstance(loss_function, FFTLoss) else loss_function(z_hat[1:, ...], z_hat_next)
        ...
        with torch.no_grad():
            for val_images in tqdm(val_loader, desc="Validating"):
                ...
                if mode == "single_ts_train":
                    ...
                    val_L_2 = loss_function(z_hat, z_hat_next, use_fft=False) if isinstance(loss_function, FFTLoss) else loss_function(z_hat, z_hat_next)
                    ...
                    val_L_2 = loss_function(z_hat[1:, ...], z_hat_next, use_fft=False) if isinstance(loss_function, FFTLoss) else loss_function(z_hat[1:, ...], z_hat_next)
