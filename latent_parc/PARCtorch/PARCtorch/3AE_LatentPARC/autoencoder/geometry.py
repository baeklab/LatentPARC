# https://github.com/Gabe-YHLee/IRVAE-public/tree/main
...
def relaxed_distortion_measure(func, x, eta=0.2, metric='identity', create_graph=True):
    if metric == 'identity':
        bs = len(x)
        x_perm = x[torch.randperm(bs)]

        if eta is not None:
            alpha = (torch.rand(bs) * (1 + 2*eta) - eta).to(x.device)
            alpha = alpha.view(bs, *([1] * (x.dim() - 1)))  # Broadcast to (B, 1, 1, 1)
            x_aug = alpha * x + (1 - alpha) * x_perm
        else:
            x_aug = x

        v = torch.randn_like(x)  # Shape: (B, C, H, W)
        Jv = torch.autograd.functional.jvp(func, x_aug, v=v, create_graph=create_graph)[1]
        TrG = torch.sum(Jv.view(bs, -1) ** 2, dim=1).mean()

        JTJv = torch.autograd.functional.vjp(func, x_aug, v=Jv, create_graph=create_graph)[1]
        JTJv = JTJv.view(bs, -1)
        TrG2 = torch.sum(JTJv ** 2, dim=1).mean()
