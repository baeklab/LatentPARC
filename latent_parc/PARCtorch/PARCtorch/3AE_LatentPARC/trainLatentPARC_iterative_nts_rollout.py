...
for num_ts in [4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4]:
    n_ts = num_ts
    
    epoch_list = [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000]
    
    n_epochs = epoch_list[num_ts-2]
    ...
    weights_name=f"iterative_nts_{num_ts}_rollouttrain_layers_3_8_latent_8_NOpurpleloss_FROZEN_AE_constLR_e5_NONOISE_bs{batch_size}_{i}x"
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.9, patience=2000, verbose=True)

    #  training model
    model = LatentPARC(model_init, optimizer, save_path, weights_name)
    
    if i > 1:
        weights_path = f"iterative_nts_{num_ts}_rollouttrain_layers_3_8_latent_8_NOpurpleloss_FROZEN_AE_constLR_e5_NONOISE_bs{batch_size}_{i-1}x_{n_epochs}.pth"
    ...
    log_dict = model.train(criterion, epochs=n_epochs, image_size = [128, 256], n_channels=3, device=device, 
